...
class CampSummaryBloc extends Bloc<CampSummaryEvent, CampSummaryState> {
  ...
  Future<void> _placeCampOrder(
      PlaceOrderCampSummaryEvent event, Emitter<CampSummaryState> emit) async {
    try {
      if (!(await Connectivity().isConnected)) {
        emit(state.copyWith(
          finalPaymentDone:false,
      ...
      var academyId = await getIt<SharedPrefs>().getString("selected_academyid");
      final response =
      await _campUsecase.placeOrderExecute({"academy_id":academyId});
      response.fold((failure) {
        emit(state.copyWith(
            error: failure.message,
            isError: true,
